...
const App = () => {
  const testDataStore = async () => {
    try {
      let jobInfo = (await DataStore.query(ContractorJobInfo, info => info.jobId.eq('dummy-id')))[0];
      ...
      jobInfo = (await DataStore.query(ContractorJobInfo, info => info.jobId.eq('dummy-id')))[0];
